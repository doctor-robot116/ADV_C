...
// Функция для подсчета общего объема выделенной памяти
size_t totalMemoryUsage(list *head)
{
    size_t mem = 0; // Инициализируем общий объем памяти 0
    while (head)
    {
        mem += head->size; // Суммируем размер каждого блока
        head = head->next; // Переходим к следующему элементу
    }
    return mem; // Возвращаем общий объем памяти
}
...
int main(void)
{
    ...
    printf("Total memory allocated: %zu\n", totalMemoryUsage(memlist)); // Выводим общий объем выделенной памяти

    // Освобождаем память с помощью модифицированного free
    free(p);
    free(ar);
    free(ar2);
    findMaxBlock(memlist); // Ищем максимальный блок
